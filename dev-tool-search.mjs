 ...
 * str_replace_editor + the discovery tools + the Biomni core tools)
 * instead of dumping the whole Standard catalog at once. This plugin
...
const UNLOCKABLE_INDEX = [
  ...
  'biomni_data — search the local Biomni data lake',
  'biomni_python — run arbitrary Python inside the local Biomni venv (direct access to all 218 Biomni tool functions)',
]
...
export function apply(ctx) {
  ctx.tools.register({
    ...
    description: [
      ...
      '',
      'This session starts with a minimal resident set: bash, str_replace_editor, skill_search, skill_load, biomni_status, biomni_tools, biomni_run, biomni_know_how. Everything else is unlocked on demand through this tool.',
      '',
