 ...
 * (`dev_tool_search`, `skill_search`, `skill_load`) PLUS any preset-specific
 * `residentTools` (the Biomni core set in this preset) plus whatever the model
...
export function apply(ctx, config) {
  ...
  // Preset-specific tools always resident after promotion (biomni core set).
  // Empty is valid: the base anchored behavior stays unchanged.
  ...
  ctx.on('system-prompt/assemble', async (_assembly, context, next) => {
    ...
    try {
      ...
      if (status.promoted) {
        ...
        // discovery tools + preset-specific resident tools + whatever the
        // model explicitly unlocked via dev_tool_search — instead of dumping
        // the whole Standard catalog at once (the post-promotion regression
        // fix; see the header note).
